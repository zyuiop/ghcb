...
impl<T: ChannelManager> IoIoHandler<T> {
	...
	fn call_from_instruction<'a>(
		...
		let out = match instruction_data.operation() {
			...
			op @ (KnownOpcode::IoInByteImm | KnownOpcode::IoInWordsImm | KnownOpcode::IoOutByteImm | KnownOpcode::IoOutWordsImm) => {
				let port = unsafe { instruction_data.read_immediate(1)[0] as u16 };
				let op = match op {
					KnownOpcode::IoInByteImm => IoIoOperation::ByteIn(unsafe { cast_ref(&mut registers.rax) }),
					KnownOpcode::IoInWordsImm if operand_16bits => IoIoOperation::WordIn(unsafe { cast_ref(&mut registers.rax) }),
					KnownOpcode::IoInWordsImm => IoIoOperation::DblWordIn(unsafe { cast_ref(&mut registers.rax) }),
					KnownOpcode::IoOutByteImm => IoIoOperation::ByteOut((registers.rax & 0xff) as u8),
					KnownOpcode::IoOutWordsImm if operand_16bits => IoIoOperation::WordOut((registers.rax & 0xffff) as u16),
					KnownOpcode::IoOutWordsImm => IoIoOperation::DblWordOut((registers.rax & 0xffff_fffff) as u32),
					_ => unreachable!()
				};

				IoIoRequest::new(port, op)
			}
			op @ (KnownOpcode::IoInByteDx | KnownOpcode::IoInWordsDx | KnownOpcode::IoOutByteDx | KnownOpcode::IoOutWordsDx) => {
				let port = (registers.rdx & 0xffff) as u16;
				let op = match op {
					KnownOpcode::IoInByteDx => IoIoOperation::ByteIn(unsafe { cast_ref(&mut registers.rax) }),
					KnownOpcode::IoInWordsDx if operand_16bits => IoIoOperation::WordIn(unsafe { cast_ref(&mut registers.rax) }),
					KnownOpcode::IoInWordsDx => IoIoOperation::DblWordIn(unsafe { cast_ref(&mut registers.rax) }),
					KnownOpcode::IoOutByteDx => IoIoOperation::ByteOut((registers.rax & 0xff) as u8),
					KnownOpcode::IoOutWordsDx if operand_16bits => IoIoOperation::WordOut((registers.rax & 0xffff) as u16),
					KnownOpcode::IoOutWordsDx => IoIoOperation::DblWordOut((registers.rax & 0xffff_fffff) as u32),
					_ => unreachable!()
				};
